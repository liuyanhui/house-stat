...
| 目录 | 作用 | 入 git |
|------|------|--------|
| `data/` | 官方自爬 CSV（可靠基准，逐月累积）；`raw/` 存原始 HTML（不进 git） | ✓（raw/ 除外） |
...
## 数据覆盖状态（截至 2026-08-24，官方自爬）

| 文件 | 覆盖 | 说明 |
|---|---|---|
| resale_monthly | 2025-01 ~ 2026-07（19 月） | 全市月度，主序列 |
| resale_daily | 2025-04-21 ~ 2026-08-23 | 周度聚合来源；2026-07-25/26 为估算（见约定 8） |
| district_monthly | 2025-12 ~ 2026-07 | 各区月度 |
| area_monthly | 2025-12 ~ 2026-07，缺 2026-04 | 2026-04 永久丢失（见约定 7）；发布侧列 2026-07 起有数据 |
| price_monthly | 2026-03 ~ 2026-07 | 发布数据全占位（官方行为），已从报告移除 |
| agency_monthly | 2025-12 ~ 2026-07，缺 2026-05/06 | 经纪机构排行；05/06 因页面删"发布套数"列致解析断流而丢失（2026-08 修复，见约定 9） |
| new_daily | 2026-04-29 ~ 2026-08-23 | 商品房每日；2026-08-23 起认购/签约 4 表字段全量解析 |
...
9. **解析器定位一律按表头/表标题文本，不做位置索引、不做行内关键字二次确认**（页面改版多次踩坑：面积表"发布套数"误存为"成交套数"、经纪机构表删列后断流 3 个月、商品房日表标题独占一行导致 `'期房' in str(row)` 永假、`soup.find_all(text=True)` 会命中 `<script>` 文本定位到容器表）。
10. **防断流三件套**（2026-08 加入）：`data/raw/` 原始 HTML 按日存档（页面只显示最新一天/一月，错过无法回补）；`check_monthly_feeds` 断流门（月度表解析为空且页面月份更新 → 非零退出）；`check_daily_freshness` 滞后告警。`validate` 另含住宅≤总计、日月对账规则；历史存量异常走 `KNOWN_ISSUES` 白名单（当前 1 条：2025-01 总面积 < 住宅面积，首爬即如此无从修正）。
